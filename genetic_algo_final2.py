...
class genetic_algo_for_clusters(object):
    ...
    def get_summary_distribution_matrix(self,sample_df):
        ...
        """
        bins_speed = [i for i in range(-1,100,20)];bins_speed.append(140)
        bins_accel=[i for i in range(-3,4,1)];bins_accel.append(115);bins_accel.insert(0,-360)
        ...
        matrix_class_cycle=((matrix_class_cycle/total).round(5))*100
    ...
    def get_summary_vector(self, sample_df):
        ...
        cycle_vector=base_parameters.values
        return cycle_vector
    ...
    def score_and_validate_cycle(self, sample_cycle):
        ...
        obj_score = np.nan
        matrix_score = np.nan
        pickle_path_save=os.path.join(r'output\pickle_files\target_vector',(str(self.pickle_genetic_dict['cycle_duration'])+'_stops_perkm.pkl'))
        if os.path.isfile(pickle_path_save):
            ...
            print('pickel file not present in score and validate cycle')
        ## cycle duration
        ## assuming raw data is sampled at 0.1 second frequency 
#        print(sample_cycle)
        seq_duration = sample_cycle.shape[0]/600 
...
#            microtrip_id_seq = self.sample_microtrip_from_cluster_seq(sample_seq)
            ...
            if ((sample_cycle is not None) and (microtrip_id_seq is not None)):
                ...
                _, validity_check_ = self.score_and_validate_cycle(sample_cycle)
    
                
                # check sequence validity by total duration
                if validity_check_:
                    pop_samples.append((sample_seq, microtrip_id_seq,sample_cycle))
#                    print('length of pop samples : ', len(pop_samples))
    ...
    def _compute_objective(self, sample_seq, microtrip_id_seq,sample_cycle):
        ...
        if self.validity_check(sample_seq, microtrip_id_seq, sample_cycle):
            obj_score, valid_seq = self.score_and_validate_cycle(sample_cycle)
...
#                print('in condition : ',valid_seq_check)
    ...
    def main(self,filtered_raw_df,filtered_agg_df,validity_matrix,num_iter):
        ...
        self.validity_matrix=validity_matrix
        print(self.raw_df.columns,self.agg_df.columns)
        population=()
...
#                print('mutation iteration : ',j)
            ...
            if last_best_score != best_score: 
                performance_check_dict[itr] = (last_best_score, best_seq, best_mt_id)
